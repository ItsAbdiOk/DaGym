...
extension VoiceLogController {
    ...
    func apply(
        spec: LogSetSpec, entryIndex: Int, session: WorkoutSession, store: WorkoutStore,
        ...
        for values in spec.sets {
            ...
            lastSummary = Self.summary(
                ...
                reps: session.exercises[entryIndex].sets[setIndex].reps
            )
        }
    ...
    func applyCompleteOnDeck(
        ...
        let summary = Self.summary(
            exerciseName: session.exercises[entryIndex].exercise.name, weightKg: set.weightKg, reps: set.reps
        )
