...
/// the type constants, `runQuery`/`quantitySamples`/`latestQuantitySample`, `bodyMassObserver`/
/// `workoutObserver`) since `private` is file-scoped and this is a separate file.
extension HealthKitStore {
    func latestBodyFatPercentage() async throws -> HealthSample? {
        try await latestQuantitySample(type: Self.bodyFatType) { quantity in
    ...
    func observeBodyMassChanges(onChange: @escaping @Sendable () -> Void) async throws {
        guard isAvailable, bodyMassObserver == nil else { return }
        try await store.enableBackgroundDelivery(for: Self.bodyMassType, frequency: .immediate)
        let query = HKObserverQuery(sampleType: Self.bodyMassType, predicate: nil) { _, completion, _ in
            onChange()
            completion()
        }
        bodyMassObserver = query
        store.execute(query)
    }
