...
if [ -z "$DEVICE_ID" ]; then
    ...
    xcrun devicectl list devices --json-output /tmp/dagym-devices.json >/dev/null 2>&1 || true
    DEVICE_ID=$(python3 -c "
import json, sys
try:
    devices = json.load(open('/tmp/dagym-devices.json'))['result']['devices']
except Exception:
    sys.exit(0)
for d in devices:
...
DERIVED=/tmp/dagym-device-build
echo "Building..."
xcodebuild -project DaGym.xcodeproj -scheme DaGym \
